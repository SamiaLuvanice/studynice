...
export function StatCard({
  ...
  return <div className={cn("stat-card animate-fade-in", variant === 'primary' && "border-primary/20 bg-gradient-to-br from-primary/5 to-primary/10", variant === 'streak' && "border-streak/20 bg-gradient-to-br from-streak/5 to-accent/10", className)}>
      <div className="flex items-start justify-between">
        ...
        {icon && <div className={cn("flex h-10 w-10 items-center justify-center rounded-xl bg-primary-foreground", variant === 'primary' && "bg-primary/10 text-primary", variant === 'streak' && "bg-streak/10 text-streak", variant === 'default' && "bg-secondary text-muted-foreground")}>
            {icon}
          </div>}
      </div>
    </div>;
